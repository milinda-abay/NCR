...
--Mace within 30 days
    SELECT
        SUM(macenew) mace30,
        COUNT(dop) AS Cases,
        CAST(SUM(macenew) AS FLOAT) / COUNT(dop) * 100 MaceWithin30Days,
        'All' AS [State]
    FROM
        (
        SELECT
...
UNION ALL
    ...
    GROUP BY
        [state]
--30 Days Mace excluding Shock/OHCA  All
    SELECT
        SUM(macenew) mace30,
        COUNT(dop) AS Cases,
        CAST(SUM(macenew) AS FLOAT) / COUNT(dop) * 100 MaceWithin30Days,
    ...
    WHERE
        ...
        AND shocksohca = 0
UNION ALL
    --30 Days Mace excluding Shock/OHCA  State
    SELECT
        SUM(macenew) mace30,
        COUNT(dop) AS Cases,
        CAST(SUM(macenew) AS FLOAT) / COUNT(dop) * 100 MaceWithin30Days,
    ...
    GROUP BY
        [state]
--Numbers for the funnel plot mace30
--  for each site
    SELECT
        SUM(macenew) mace30#,
        COUNT(dop) AS Cases,
        SUM(CAST(macenew AS FLOAT)) / COUNT(dop) * 100 maceWithin30Days,
        'All' AS [State],
        'All' AS [ncrhid]
    FROM
        (
...
UNION ALL
    ...
    GROUP BY
        [state],
        [ncrhid]
--Numbers for the funnel plot mace30 without shock or OHCA
--  for each site
    SELECT
        SUM(macenew) mace30excshocksohca#,
        COUNT(dop) AS Cases,
        SUM(CAST(macenew AS FLOAT)) / COUNT(dop) * 100 MaceWithin30Days,
        'All' AS [State],
        'All' AS [ncrhid]
